  ...
  template: `
    <div class="min-h-[85vh] flex items-center justify-center px-4 py-12 relative overflow-hidden bg-muted/20">
      ...
      <div class="absolute top-0 start-0 w-full h-full overflow-hidden pointer-events-none z-0">
        <div class="absolute -top-[10%] -start-[5%] w-[40%] h-[50%] rounded-full bg-primary/5 blur-[100px] animate-pulse" style="animation-duration: 4s;"></div>
        <div class="absolute top-[40%] -end-[10%] w-[35%] h-[40%] rounded-full bg-primary/8 blur-[100px] animate-pulse" style="animation-duration: 6s;"></div>
      </div>
      ...
      <div class="w-full max-w-md bg-white/70 backdrop-blur-xl p-8 md:p-10 rounded-2xl shadow-xl border border-white/50 relative z-10 transition-all duration-300 hover:shadow-2xl">
        ...
        <div class="text-center mb-10">
          ...
          <p class="text-muted-foreground font-inter text-sm">{{ 'auth.login.subtitle' | t }}</p>
        </div>
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-6">
          <div class="space-y-2">
            ...
            @if (form.get('email')?.touched && form.get('email')?.hasError('required')) {
              <p class="text-red-500 text-xs mt-1.5 font-medium">{{ 'auth.login.emailRequired' | t }}</p>
            }
            @if (form.get('email')?.touched && form.get('email')?.hasError('email')) {
              <p class="text-red-500 text-xs mt-1.5 font-medium">{{ 'auth.login.emailInvalid' | t }}</p>
            }
          </div>
          ...
          <div class="space-y-2">
            ...
            @if (form.get('password')?.touched && form.get('password')?.hasError('required')) {
              <p class="text-red-500 text-xs mt-1.5 font-medium">{{ 'auth.login.passwordRequired' | t }}</p>
            }
          </div>

          <div class="flex justify-end pt-1">
            <a routerLink="/forgot-password" class="text-sm font-medium text-primary hover:text-primary-dark transition-colors duration-200">
          ...
          >
            @if (loading()) {
              ...
              <span>{{ 'auth.login.loading' | t }}</span>
            } @else {
              <span>{{ 'auth.login.submit' | t }}</span>
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 rtl:rotate-180" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M12.293 5.293a1 1 0 011.414 0l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-2.293-2.293a1 1 0 010-1.414z" clip-rule="evenodd" /></svg>
            }
          </button>
        </form>
        ...
        <div class="mt-8 pt-8 border-t border-border">
          <p class="text-center text-sm text-muted-foreground font-inter">
            ...
            <a routerLink="/register" class="text-primary font-semibold hover:text-primary-dark transition-colors ms-1">{{ 'auth.login.createAccount' | t }}</a>
          </p>
        </div>
      </div>
    </div>
...
export class LoginComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.seoService.updatePage({
      title: this.t.get('login.seo.title'),
      description: this.t.get('login.seo.description'),
      url: '/login',
    });
  }
  ...
  onSubmit(): void {
    ...
    this.authService.login(this.form.getRawValue()).subscribe({
      next: (response) => {
        ...
        const destination = returnUrl || (this.authService.isAdmin() ? '/admin' : '/');
        // replaceUrl removes the login page from history to prevent
        // back-button returning to the login form after successful auth
        this.router.navigateByUrl(destination, { replaceUrl: true });
      },
      error: (err) => {
        this.loading.set(false);
        this.errorMessage.set(err?.message || this.t.get('auth.login.invalidCredentials'));
      },
    });
  }
}
